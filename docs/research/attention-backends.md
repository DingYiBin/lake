...
lake 的 `ModelRunner` 已是 SGLang 形态：单一类 + HF `architectures`→model registry + `build_attn_backend(name)` 注册表 + 无子类化。**不需要引入 vLLM 式的 CPU/GPU runner 分裂**。平台差异全进 attention backend；runner 内必要时加 `if device == "cpu"` 分支（如 warmup 跳过 graph capture），但不子类化。
...
| model runner 形态 | 保持 SGLang 形态（单一 `ModelRunner` + architecture registry + attn 注册表），**不**按平台子类化 | vLLM V2 时代 CPU runner 缩到 17 行，正在向单类收敛 |
