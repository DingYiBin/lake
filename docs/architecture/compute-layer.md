...
**C11 状态（2026-08-04）**：`InputBuffers` / `AttentionMetadata` 热路径已迁 **torch 固定 buffer**（默认 `device=cpu`；`cuda` 经 pin staging + non_blocking H2D）；`block_table` 为 `[max_reqs, max_blocks]` + `block_table_lens`。计划与字段可行性见 [`input-batch-device.md`](input-batch-device.md)。`ReadyHandle.slot_mapping_by_req` / `InMemoryAgent` 逻辑表仍 host 产出后 materialize。decode / TARGET_VERIFY 的 `read_set`/`write_set` 契约不变。`SchedulerOutput.req_query_*` 仍为 runner 几何权威。CUDA graph capture、device Triton pack（P3）仍后置。
