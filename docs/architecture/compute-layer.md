...
| D4 | **Attention 后端与 metadata 边界** | **C8 初版已定**：`attn/metadata.py::AttentionMetadata` + `ReadyHandle.block_table_by_req`（agent 出表、runner 只读）；`forward_queries` 残差路径；真固定地址 tensor / paged kernel 仍待生产。**2026-08-02 补**（见 [`../research/attention-backends.md`](../research/attention-backends.md)）：① metadata 保持 vLLM 形态（独立 `AttentionMetadata` + `build_attn_metadata`），不学 SGLang 并进 ForwardBatch；② GPU 生产走 Triton paged varlen kernel（`TritonAttentionBackend`），CPU/dev/test 走纯 torch SDPA 方案 A（`TorchAttentionBackend.forward_varlen`，per-request 循环 + gather paged + SDPA，对齐 SGLang `torch_native`）；③ **不装 vllm**——单体 serving 系统非 library，与 Q1/Q2 冲突；GPU 若借 flash-attn 须用 `vllm-project/flash-attention` fork（带 `block_table` 签名），stock `flash-attn` 不接 `block_table` 做 不了 paged；④ `ModelRunner` 保持单类 + `model_backend` 字段 + 注册表，**不**按平台子类化（vLLM 子类化是历史包袱，V2 已向单类收敛）；⑤ `ForwardMode.MIXED` 不单独 `forward_mixed`——调度侧把 decode 标成 `extend_len=1`，runner 侧 varlen 自然吃（SGLang CUDA 同款，仅 NPU 因 kernel 限制另写 `forward_mixed`）；⑥ **混部共享进程级并行度，prefill/decode 差异化并行必须走 PD 分离**（Router 选路），不能在混部里做——并行度是进程级配置，MIXED 同 forward 同 communicator | 对照 vLLM `AttentionMetadataBuilder`；详见 [`../research/attention-backends.md`](../research/attention-backends.md) |
