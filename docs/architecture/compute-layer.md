...
计划与字段可行性、阶段划分见 [`input-batch-device.md`](input-batch-device.md)（P0/P1 已落：`InputBuffers` 固定 tensor、`AttentionMetadata` device 视图）。

### TP / DP 通讯域（ParallelConfig）

对齐 vLLM mesh：`DP × PP × TP`（PCP=1）。配置与建组骨架已落在计算层：

| 模块 | 路径 | 说明 |
|------|------|------|
| `ParallelConfig` / `RoleConfig` | `python/lake/engine/config/{parallel,role}.py` | 计算层配置权威路径（对齐 vLLM `vllm/config/`）；env：`LAKE_TP_SIZE` / `LAKE_DP_SIZE` / … |
...
> 对照全集见 [`../research/scheduler-worker-interface.md`](../research/scheduler-worker-interface.md)。定稿原则：**调度几何与信封外形偏 vLLM**（`num_computed_tokens` + 本步 `num_scheduled_tokens` + 增量 new/cached + `execute_model(SchedulerOutput)`）；**不**用 SGLang 式 Prefill/Extend/Decode/PREBUILT **分相状态机**驱动执行；`ForwardMode` 仅为由几何派生的批标签（图/日志）；DP 字段（`global_num_tokens*`）仍可挂 Scheduler；**KV 句柄两边都不照搬**。
...
| 量 | 含义 |
|----|------|
| `num_computed_tokens` | 已有 KV 覆盖的 token 上界（半开） |
| 本步 `num_scheduled_tokens[req]` | 本步要**新算**的 token 数 |
| prompt 残差 | `computed < prompt_len` → schedule `prompt_len - computed`，write `[computed, prompt_len)` |
| 生成 | `computed >= prompt_len` → schedule 1（或 spec K）；整段本地命中只把 `computed` 提到 `prompt_len`，无独立 PREBUILT 态 |
| prepare | 按 read/write **几何**保证 L0（触及未 ensure 的 prompt 区间则补拉/Get） |

### `ForwardMode`（派生标签，非执行态）

| 值 | 何时派生 | 图/eager |
|----|----------|----------|
| `EXTEND` | 本步仍在算 prompt 残差 | eager |
| `DECODE` | 本步在生成且无 spec verify | graph（生产） |
| `MIXED` | 同批两种几何 | eager |
| `IDLE` | 空步 / DP 陪跑 | 随共识 |
| `TARGET_VERIFY` | `scheduled_spec_decode_tokens` 非空 | graph/eager |
| `PREBUILT` | **废弃**（勿再 schedule；整段命中走生成几何） | — |
...
- **参考**：vLLM `Scheduler.schedule` / `SchedulerOutput` / `num_computed_tokens` + `num_scheduled_tokens`；DP 同步字段写入点可对照 SGLang `prepare_mlp_sync_batch_raw`。
- **值得参考**：vLLM「计算同构、差别在本步 token 数与图策略」；增量信封便于扇出多卡。
- **关键差异**：KV 句柄归池（`read_set`/`write_set`）；不采用 SGLang Prefill/Extend/Decode/PREBUILT 执行态机；`ForwardMode` 仅派生标签。
...
| InputBatch / buffer | `InputBuffers` 预分配 GPU tensor，`InputBatch` 含 positions、query_start_loc、logits_indices、padding、spec 宽度 | `InputBuffers` 已 device tensor（默认 cpu；cuda 经 pin H2D）；`InputBatch` 仍 host dict | 见 [`input-batch-device.md`](input-batch-device.md)；P2/P3 待办 |
...
| ModelRunner | `GPUModelRunner` 能 `load_model`、`initialize_kv_cache`、`prepare_inputs`、`execute_model`、`sample_tokens`、`_dummy_run`；另有 sampler/structured/spec/PP/graph 子模块 | `ModelRunner` 已拆 `prepare_inputs` / `prepare_attn` / `sample_tokens`；选模走 HF `architectures`→registry，测试用 `load_format=dummy` + 缩配置（无 `model_backend=mock`） | 需接入真权重加载、runner buffer 生命周期、sample/structured 拆分，不引入 runner 内请求表 |
...
**C11 状态（2026-08-04）**：`InputBuffers` / `AttentionMetadata` 热路径已迁 **torch 固定 buffer**（默认 `device=cpu`；`cuda` 经 pin staging + non_blocking H2D）；`block_table` 为 `[max_reqs, max_blocks]` + `block_table_lens`。计划与字段可行性见 [`input-batch-device.md`](input-batch-device.md)。**P2 已落**：host `InputBatch` 由 dict-by-req_id 收敛为**行稠密 list**（平行于 `req_ids`，对齐 vLLM V2 `InputBatch` 行索引形态），`add_request` 按行追加、`index_of` 反查、`materialize` 按行迭代；`prepare_attn` 由行稠密 batch 构一过性 dict 视图喂 `build_attn_metadata`（builder 仍取 dict 几何权威）。**P3.1 已落**：固定 shape padding（graph capture 地基，不实现 graph 本身）——`RoleConfig.pad_num_reqs/tokens` + env、`InputBuffers.set_padding`/`effective_*`、`AttentionMetadata` padded shape + `is_padding` + `actual_*`、CPU 后端跳 pad req；默认 off，行为不变。`ReadyHandle.slot_mapping_by_req` / `InMemoryAgent` 逻辑表仍 host 产出后 materialize。decode / TARGET_VERIFY 的 `read_set`/`write_set` 契约不变。`SchedulerOutput.req_query_*` 仍为 runner 几何权威。CUDA graph capture、device Triton pack（P3 主体）仍后置。

**C12 状态（2026-08-01）**：已落 `ModelRunner.load_model` / `warmup` / `ModelRunnerStatus`，`WorkerEngine.start()` 在 Warm 阶段执行 load+warmup，`CapacitySignal` 上报 `served_model_name` / loaded / warmed；权重 pin 以回调形式接入，保持权重所有权在池侧。加载源统一命名为 `model_path`（HF repo id 或本地目录），对外服务名为 `served_model_name`，默认 `"model"`，不从 `args.model` / 本地路径派生，避免暴露部署路径；KV Pool/proto 里的 `model_id` 字段暂由 `PoolIface` 兼容映射为 served name，后续按 #49 讨论是否改为独立 KV namespace。启动与请求构造不再设置 `default_model_id`：加载时先用 `transformers.AutoConfig.from_pretrained(model_path)` 读取 HF config，让缺失/非法加载源由 HF config/loader 自然抛错；随后读取 `config.architectures`，通过 `ModelRegistry.resolve_model_cls()` lazy import 模型类；不再维护 `model_id -> Qwen3Config` 的硬编码映射。模型创建与权重加载收敛到 `lake/engine/model_executor/models/loader.py`：`BaseModelLoader.load_model()` 统一 `model_cls(config) -> load_weights(model)`；`DummyModelLoader` 只实现 dummy 权重注入；`DefaultModelLoader` 预留真实 safetensors/HF 权重读取边界（当前显式未实现）。`ModelRunner.load_model()` 只调用 registry/loader、挂载返回模型、更新状态/回调，对齐 vLLM `ModelConfig.hf_config.architectures -> ModelRegistry.resolve_model_cls -> BaseModelLoader.load_model` 的边界。Qwen3 config 不再自建 dataclass，也不在 lake 写死 Qwen3-0.6B 常量；测试默认使用 HF repo id `Qwen/Qwen3-0.6B` 作为 example，不在仓库保存模型文件，需本地离线/加速时可用 `LAKE_TEST_QWEN3_MODEL_PATH=~/models/Qwen/Qwen3-0.6B` 指向已下载目录；`3rdparty/transformers` 仅用于源码参考，不进入安装路径。Qwen 系列模型文件已移入 `engine/model_executor/models/qwen/`；Qwen3 module tree 已补齐 vLLM packed layout：`embed_tokens`、逐层 `self_attn.qkv_proj/o_proj/q_norm/k_norm/attn/rotary_emb`、`mlp.gate_up_proj/down_proj`、两处 RMSNorm、最终 `norm` 与 tied/untied `lm_head`；参数放在 `meta` device 以避免骨架阶段分配 0.6B 权重。attention backend 新增纯 Torch SDPA 路径（`lake/engine/model_executor/layers/attentions/attention.py::CpuAttentionBackend.forward_tensors`（原 `TorchAttentionBackend`，2026-08-02 改名），支持 `[B,H,T,D]`、causal mask、GQA KV head repeat），Qwen3 的 `Qwen3PagedAttention` 在收到 tensor q/k/v 时使用该后端。Python 计算层以 Torch 为基础依赖，默认后端切为 `qwen3`；这与 vLLM / SGLang 的生产计算路径一致：两者都在包依赖与 runner/model 热路径中把 Torch 当作硬依赖，而非可选模型插件。lake 仍保留轻量 import 边界：`engine` / `runtime` import 路径不会顶层触发 Torch，只有实际初始化 Qwen3 backend 时才 import `Qwen3ForCausalLM(nn.Module)`。模型分层按 Transformers/vLLM 形态保留 `Qwen3Model` backbone、`forward`、`compute_logits`、`load_weights(weights)` 边界，dummy 不污染模型 API。当前 deterministic decode 在 runner 层占位；TinyLM / TinyMTPDrafter 已移除，投机执行等待后续真实 draft/spec 模型接入。warmup 复用 `dummy_run()`，不触发 `pool.prepare_step` / `pool.done`。

**C11+ 状态（2026-08-02）**：`CpuAttentionBackend.forward_varlen` 已落（方案 A，对齐 SGLang `torch_native`，原 `TorchAttentionBackend` 改名而来）——per-request 循环 + block-paged gather KV + SDPA：extend（q_len>1）pad query 到 seq_len + is_causal=True + slice 新段；decode（q_len=1）不 pad + is_causal=False（full-visible）。入参 `q [num_tokens,H,D]` / `k_cache,v_cache [total_slots,Hkv,D]`（池 L0 arena 句柄，引擎只读）/ `AttentionMetadata`（`block_table_tensor` block 级 + `query_start_loc` + 新增 `seq_lens_ordered`）。GQA 经 `repeat_interleave` 对齐 q heads。单测覆盖 extend / decode / mixed batch / chunked extend，对照手动 pad+causal+slice 参照。这是 CPU/dev/test 路径；GPU 生产路径新增 `FlashAttn2Backend`（上游 `flash-attn` 包 FA2 paged varlen，`causal=True` 统一 prefill/extend/decode，GQA 原生，非入图——无 `out=` out-param），已注册 `build_attn_backend("fa2")`，待 GPU 环境装 `flash-attn` 实测（含 block_size<256 兼容性确认）。`Qwen3PagedAttention.forward` 已接 metadata 分派：`attn_meta` 非空走 `forward_varlen`（paged，`q [num_tokens,H,D]` + `k/v` 为池 L0 arena 句柄），为空走 `forward_tensors`（`[B,H,T,D]`，dev/dummy）；forward 内不做平台判定。**后端管理对齐 vLLM/SGLang**：模型层（`qwen3.py`）**不 import 任何具体后端**——`ModelRunner` 经注册表 `build_attn_backend(attn_backend_name)` 建一个后端实例（`attn_backend_name` 由 `RoleConfig`/`LAKE_ATTN_BACKEND` 配置，默认 `cpu`），经 loader→`Qwen3ForCausalLM`→`Qwen3Model`→`Qwen3DecoderLayer`→`Qwen3Attention`→`Qwen3PagedAttention` 构造期注入（`backend=` 句柄，`block_size=`）；loader 用 `inspect.signature` 兼容不接受 `attn_backend` 的自定义/测试模型类。对照：vLLM 每层 `Attention.__init__` 调 `get_impl_cls()` 实例化自己的 impl、metadata/kv_cache 经 forward context；SGLang runner 持单后端实例、层经 `get_attn_backend()` 取——lake 取二者折中：runner 持单实例（SGLang 式，后端无状态）+ 构造期注入（vLLM 式，免 forward context 改造）。单测覆盖经模型层分派的 mixed batch（对照手动 pad+causal 参照 + 后端直连等价）。**后端文件拆分**：原 `attentions/attention.py` 拆成 `attentions/backends/` 子包——`base.py`（`AttentionBackend` 协议）/ `cpu.py`（`CpuAttentionBackend`）/ `fa2.py`（`FlashAttn2Backend`）/ `ref.py` / `triton.py` / `registry.py`（`build_attn_backend`，懒导入——import 包不拉入任何 kernel/flash-attn），`attentions/__init__.py` 懒委托到子包。当前 dummy `Qwen3Attention.forward` 仍不产真实 q/k/v 张量——真正经模型 forward 调起本分派待后续写实 forward。
...
**C16b 状态（2026-08-04）**：batched paged forward 路径已落——新增 `attentions/context.py`（`ForwardContext` + `forward_context` 上下文管理器，对齐 vLLM `set_forward_context`：runner 调模型 forward 前注入 `attn_meta` + per-layer `kv_caches`，各 `Qwen3Attention` 按 `layer_idx` 取自己那层 arena 句柄，模型 API 不显式传 KV）。`InputBuffers` 增 `block_size`（默认 8，arena 总槽 = `max_num_blocks * block_size`）。`ModelRunner._allocate_kv_caches` 在 `load_model` 后分配 mock per-layer `(k_cache, v_cache) [total_slots, num_kv_heads, head_dim]`（生产由存储池放置 HBM，dummy/test 由 runner 持句柄；非 Qwen3 结构返回空列表回退）。`Qwen3Attention.forward` 增 paged 分支：RoPE 后按 `meta.slot_mapping` 把新 k/v 写入 arena（pad 槽 -1 屏蔽），调 `forward_varlen` 读 paged 前缀 KV。`_forward_model` 切到 batched paged：flat query `input_ids`（仅本步新算 token，非全上下文）→ forward → 取每请求末位 query token logits 采样；agent 未出 block table（测试桩）或无 arena 时回退 C16a 逐请求非分页。单测验证 paged EXTEND→DECODE 末位 logits 与逐请求非分页 forward 精确对齐（`assert_close`）。arena 归属（runner mock → 存储池）、真 HBM 放置、`ReadyHandle` 暴露 arena 句柄仍后置。
...
| `done(step_id)` | `int` | `/ PROTOCOL_ERROR` | done fence：解冻；触发满块写回路径（生产） |
...
| `PROTOCOL_ERROR` | ready/done fence 错配或重入 prepare（非「资源未就绪」） | 视为故障 → F4（gRPC `UNAVAILABLE`） |
...
pool_iface.prepare_step(plan)      # ★ 唯一点发起补拉 / 占槽 / 冻结 / 组表
        │  pull_budget_ms:
        │    0     → 同步等到齐（P3 mock）
        │    >0    → 超时 TIMEOUT；或 allow_partial_hit 缩批（丢缺块 req）
        │  默认不允许「批内缺块请求仍进算」——all-or-nothing（≠ 单请求前缀部分命中）
        ▼
...
| D3 | **角色配置 schema** | `role=prefill\|decode\|hybrid` 已定方向;未定完整启动配置(模型、TP、是否挂 drafter、arena 尺寸、上报指标标签)；C0 仅最小 `RoleConfig`（权威路径 `engine/config/role.py`） | `engine/config`；与冷启动 Warm 对齐 |
| D4 | **Attention 后端与 metadata 边界** | **C8 初版已定**：`attn/metadata.py::AttentionMetadata` + `ReadyHandle.block_table_by_req`（agent 出表、runner 只读）；`forward_queries` 残差路径；真固定地址 tensor / paged kernel 仍待生产。**2026-08-02 补**（见 [`../research/attention-backends.md`](../research/attention-backends.md)）：① metadata 保持 vLLM 形态（独立 `AttentionMetadata` + `build_attn_metadata`），不学 SGLang 并进 ForwardBatch；② GPU 生产走 FA2 paged varlen（`FlashAttn2Backend`，上游 `flash-attn` 包，`causal=True` 统一三模式、GQA 原生、非入图），CPU/dev/test 走纯 torch SDPA 方案 A（`CpuAttentionBackend.forward_varlen`，原 `TorchAttentionBackend` 改名，per-request 循环 + gather paged + SDPA，对齐 SGLang `torch_native`）；③ **不装 vllm**——单体 serving 系统非 library，与 Q1/Q2 冲突；GPU 借 flash-attn 优先上游包（FA2 varlen 已带 `block_table`），若上游不支持小 `block_size`（`with_kvcache` 要求 256 倍数）或入图需 `out=`，再回退 `vllm-project/flash-attention` fork；④ `ModelRunner` 保持单类 + architecture registry + attn 注册表，**不**按平台子类化（vLLM 子类化是历史包袱，V2 已向单类收敛）；**后端注入**：runner 经 `build_attn_backend(attn_backend_name)` 建实例（`RoleConfig.attn_backend_name`/`LAKE_ATTN_BACKEND`，默认 `cpu`），构造期沿模型树下传到 `Qwen3PagedAttention.backend`——模型层不 import 任何具体后端（对齐 vLLM `Attention`/SGLang `RadixAttention`：后端在 runner 层选定注入，层只持句柄）；lake 取 vLLM（构造期注入）+ SGLang（runner 持单实例）折中，免 forward context 改造；⑤ `ForwardMode.MIXED` 不单独 `forward_mixed`——调度侧把 decode 标成 `extend_len=1`，runner 侧 varlen 自然吃（SGLang CUDA 同款，仅 NPU 因 kernel 限制另写 `forward_mixed`）；⑥ **混部共享进程级并行度，prefill/decode 差异化并行必须走 PD 分离**（Router 选路），不能在混部里做——并行度是进程级配置，MIXED 同 forward 同 communicator | 对照 vLLM `AttentionMetadataBuilder`；详见 [`../research/attention-backends.md`](../research/attention-backends.md) |
